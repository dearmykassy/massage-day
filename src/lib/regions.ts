...
if (
  ...
  throw new Error("MASSAGE_DAY_CAPITAL_REGION_SOURCE_INVALID");
}
...
if (
  ...
  throw new Error("MASSAGE_DAY_SERVICE_REGION_SOURCE_INVALID");
}
...
function nodeForSegments(segmentsInput: readonly string[]): RegionNode | null {
  ...
  return {
    id: `massage-day-region-${segments.map(encodeURIComponent).join("-")}`,
    kind,
    rootKey,
    segments,
...
if (ACTIVE_REGION_NODES.length !== 1291) {
  throw new Error(
    `MASSAGE_DAY_ACTIVE_ROUTE_COUNT_INVALID:${ACTIVE_REGION_NODES.length}`,
  );
}
...
const ORDINAL_BY_PATH = new Map(
  ACTIVE_REGION_NODES.map((node, index) => [node.path, index]),
);

export function getRegionOrdinal(node: RegionNode): number {
  const ordinal = ORDINAL_BY_PATH.get(node.path);
  if (ordinal === undefined) {
    throw new Error(`MASSAGE_DAY_REGION_ORDINAL_MISSING:${node.path}`);
  }
  return ordinal;
}
...
export function getRootNode(rootKey: ActiveRootKey): RegionNode {
  ...
  if (!node) throw new Error(`MASSAGE_DAY_ROOT_NOT_FOUND:${rootKey}`);
  return node;
}
